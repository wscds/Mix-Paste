...
class MixPaste:
    ...
    def __call__(self, results):
        ...
        results = self._mixpaste_transform(results)
        return results

    def get_indexes(self, dataset, idx):

        target_labels = dataset.get_ann_info(idx)['labels']
        # print(target_labels)
        ...
        for i in range(self.max_iters):
            index = random.randint(0, len(dataset))
            gt_labels_i = dataset.get_ann_info(index)['labels']
            if len(gt_labels_i) != 0 and gt_labels_i[0] == target_labels[0]:
                idxs.append(index)
                if (len(idxs) == self.num_images):
                    break
        if (len(idxs) == 0):
            return idx
    ...
    def create_decaying_padding(self, height, width, decay_range):
        padding_width = min(height, width) * 0.25
        # 创建坐标网格
        y, x = np.meshgrid(range(height), range(width))

        # 计算距离中心点的距离
        distance = np.minimum(np.minimum(x, width - x - 1), np.minimum(y, height - y - 1)).T
        
        # 创建一个全零的padding
        padding = np.zeros((height, width), dtype=np.float32)

        # 在padding范围内设置逐渐减少的值
        padding[distance < padding_width] = 1 - (1 - decay_range) * (distance[distance < padding_width] / padding_width)

        # 在一定范围内保持不变
        padding[distance >= padding_width] = decay_range

        return padding[:, :, np.newaxis]

    def _mixpaste_transform(self, results):
        
        p = random.random()
        if (p > self.prob):
            return results
        assert 'mix_results' in results
        bbox = [int(results['ann_info']['bboxes'][0][i]) for i in range(len(results['ann_info']['bboxes'][0]))]

        img = results['img'][bbox[1]: bbox[3], bbox[0]:bbox[2], :].astype("float")
        out_img = img
        if self.use_mask:
            bbox2 = [int(results['mix_results'][0]['ann_info']['bboxes'][0][i]) for i in range(len(results['mix_results'][0]['ann_info']['bboxes'][0]))]
            
            img2 = results['mix_results'][0]['img'][bbox2[1]: bbox2[3], bbox2[0]:bbox2[2], :].astype("float")

            if img.shape[0] == 0 or img2.shape[0] == 0 or img.shape[1] == 0 or img2.shape[1] == 0:
                # print(results['filename'], results['mix_results'][0]['filename'])
                return results
            scale1 = img.shape[0] * 1.0 / img.shape[1]
            scale2 = img2.shape[0] * 1.0 / img2.shape[1]
            if (scale1 > 1 and scale2 < 1) or (scale1 < 1 and scale2 > 1):
                img2 = np.transpose(img2, (1, 0, 2))
            img2 = mmcv.imresize(
                img2, (img.shape[1] ,img.shape[0]))
            mask = self.create_decaying_padding(img.shape[0], img.shape[1], np.random.beta(self.alpha, self.alpha))
            out_img = img * mask + img2 * (1-mask)
            # print(mask)
            results['img'][bbox[1]: bbox[3], bbox[0]:bbox[2], :] = out_img
        else:
            for j in range(len(results['mix_results'])):
                bbox2 = [int(results['mix_results'][j]['ann_info']['bboxes'][0][i]) for i in range(len(results['mix_results'][j]['ann_info']['bboxes'][0]))]
                
                img2 = results['mix_results'][j]['img'][bbox2[1]: bbox2[3], bbox2[0]:bbox2[2], :].astype("float")
                ...
                out_img += img2
            results['img'][bbox[1]: bbox[3], bbox[0]:bbox[2], :] = out_img / (len(results['mix_results']) + 1)
